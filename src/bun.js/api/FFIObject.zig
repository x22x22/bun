...
fn getCPtr(value: JSValue) ?usize {
    ...
    if (value.isNumber()) {
        if (!std.math.isFinite(value.asNumber()) or value.asNumber() < 0 or value.asNumber() > @as(f64, @as(comptime_float, std.math.maxInt(usize)))) return null;
        const addr = value.asPtrAddress();
        if (addr >= std.heap.page_size_min) return addr;
    } else if (value.isBigInt()) {
        const addr = @as(u64, @bitCast(value.toUInt64NoTruncate()));
        if (addr >= std.heap.page_size_min and addr <= max_addressable_memory) {
            return addr;
        }
    }
